...
import { URLTagihan } from "../config/url.js";
get(URLTagihan, tabelContent);
...
function userTable(jsonParse) {
  ...
  jsonParse.forEach((element) => {
    ...
    row = rowtabel.replace("#namamahasiswa#", element.namamahasiswa).replace("#nim#", element.nim).replace("#smt#", semester).replace("#biaya_spp#", element.biaya_spp).replace("#SVG#", svgicon);
    addInner("demo", row);
  });
}
